...
namespace UnityGameFramework.Editor
{
    ...
    {
        private static string tablePath = "\\ProjectZGL\\DataTables\\";
        private static string codePath = "\\ProjectZGL\\Scripts\\DataTable\\";
        private static string codeSpace = "ProjectZGL";
        ...
        private static void HandleAllDataTables()
        {
            ...
            if (Directory.Exists(fullPath))
            {
                ...
                for (int i = 0; i < files.Length; i++)
                {
                    ...
                    LoadFile(files[i].FullName,files[i].Name);
                }
            }
        }


        static void LoadFile(string filePath,string filename)
        {
            using (StreamReader sr = File.OpenText(filePath))
            {
                ...
                sr.Close();
                sr.Dispose();
                HandleData(lines,filename);
            }

        }
        static void HandleData(List<string> info,string filename)
        {
            // string[] textOfTableNames = info[0].Split('\t');
            // string textOfTableName = textOfTableNames[1];
            filename =filename.Substring(0,filename.Length-4);
            string textOfTableName = "DR"+filename;
        ...
        static void WriteParseDataRow(StreamWriter sw,string[] names,string[] types)
        {
            ...
            sw.WriteLine("  {");
            sw.WriteLine("    string[] text = DataTableExtension.SplitDataRow(dataRowText);");
            ...
            for (int i = 2; i < names.Length; i++)
            {
                ...
                if(types[i]=="List<int>")
                {
                    sw.WriteLine(string.Format("	{0} = DataTableExtension.SplitIntList(text[index++]);", names[i]));
                }
				else if (types[i]=="List<string>")
                {
	                sw.WriteLine(string.Format("	{0} = new List<string>(text[index++].Split('*'));", names[i]));
                }
